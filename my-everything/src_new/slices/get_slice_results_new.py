...
def get_slice_info_from_dot_criterion_new(criterion, myslice_filepath="", level='function', G_slice=None, ):
    """
    Parameters:
    ...
    for node in G_slice.nodes(data=True):
        ...
        # TODO: add modification property to node_location
        ...
        if node_location not in slice_codes_dict[node_filepath]:
            ...
            logger.warning(f"Node location already exists: {node_filepath} {node_location}")
            exist_code = slice_codes_dict[node_filepath][node_location]['code']
            if exist_code != node_file_code:
                logger.warning(f"exist_code not equal to node_file_code: {node_filepath} {node_location}")
                logger.debug(f"exist_code not equal to node_file_code: {node_filepath} {node_location} {exist_code} {node_file_code}")
                if criterion['modification'] == 'ADD':
                    tail = "new"
                elif criterion['modification'] == 'DELETE':
                    tail = "old"
                else:
                    tail = "unknown"
                node_location = f"{node_location}({tail})"
...
def combine_dot_file():
    """combine all sliced dot files"""
    pass


def get_patch_stmts_meta():
    """get the meta information of the patch statements"""
    pass


def match_patch_stmts():
    """match patch statements with the node in dot graph"""
    pass


def test():
    # get all patch statements

    # get patch statement nodes (from line number)

    # del config.CODE_ROOT / config.CODE_NEW/OLD
    pass